<x-navbar>
    <h2 class="font-bold text-[40px] text-center mt-6"><span class="text-primary">{{ Str::ucfirst($categoryName) }}
    ...
    <div
        ...
        <div class="flex lg:gap-10 gap-8 items-center justify-center">
            ...
            <form action="" method="GET"
                class="flex gap-2 items-center hover:text-primary text-gray-600 hover:scale-105 transition cursor-pointer">
            ...
            <form action="" method="GET"
                class="flex gap-2 items-center hover:text-primary hover:scale-105 transition cursor-pointer text-gray-600">
                <input type="text" name="sortby" value="nb_demandes" hidden>
                <input type="text" name="ville" value="{{ $ville }}" hidden>
                <i class="fa-solid fa-clipboard-list"></i> <button>Demandes</button>
            </form>
            <form action="" method="GET"
                class="flex gap-2 items-center hover:text-primary text-gray-600 hover:scale-105 transition cursor-pointer">
                <input type="text" name="sortby" value="rating" hidden>
                <input type="text" name="ville" value="{{ $ville }}" hidden>
                <i class="fa-solid fa-star"></i> <button>Note</button>
            </form>
        </div>
        <form action="" method="GET" class="flex lg:gap-5 lg:mt-0 mt-5 gap-8 items-center justify-center">
            <h2 class="font-bold">Filtrer par :</h2>
            <input type="text" name="sortby" value="{{ $sortby }}" hidden>
            <select
                class="hover:text-primary hover:scale-105 hover:border-primary border border-gray-300 p-[1px]  rounded lg:w-40 w-36 focus:outline-0 cursor-pointer cities-list transition"
            ...
            <button
                class="bg-primary text-white text-sm leading-6 font-medium py-1 px-3 rounded hover:bg-[#7C3AED]  transition">appliquer</button>
        </form>
    </div>
